...
class VaultService : Service() {
    ...
    private val chunkSize = 8192 // 8KB chunks for memory efficiency
     ...
     */
    suspend fun encryptFileChunked(
        inputFile: File,
        outputFile: File,
    ): String {
     ...
     */
    suspend fun decryptFileChunked(
        inputFile: File,
        outputFile: File,
        iv: String,
    ) {
        withContext(Dispatchers.IO) {
            ...
            inputFile.inputStream().use { input ->
                outputFile.outputStream().use { output ->
                    val buffer = ByteArray(chunkSize + 16) // Account for GCM tag
                    var bytesRead: Int
